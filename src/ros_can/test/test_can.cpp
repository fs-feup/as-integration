...
TEST_F(RosCanTest, ControlCallback) {
    prepareOutOfRangeValues(0.3, 0.3, 1);
    ros_can_->controlCallback(control_command_);
}
...
TEST_F(RosCanTest, EmergencyCallback) {
    testServiceCall("/as_srv/emergency", EMERGENCY_CODE);
}

TEST_F(RosCanTest, MissionFinishedCallback) {
    testServiceCall("/as_srv/mission_finished", MISSION_FINISHED_CODE);
}
...
TEST_F(RosCanTest, TestImuYawAccYPublisher) {
    ...
    auto imuYawAccYSub = test_node_->create_subscription<custom_interfaces::msg::ImuData>(
            "imuYawAccY", 10, [&](const custom_interfaces::msg::ImuData::SharedPtr msg) {
                ...
                EXPECT_TRUE(isApproxEqual(msg->gyro, 1.0 * QUANTIZATION_GYRO, tolerance));
                EXPECT_TRUE(isApproxEqual(msg->acc, 2.0 * QUANTIZATION_ACC, tolerance));
            });
...
/**
 * @test This test checks if the operational status callback function is called after receiving an operational status message from ROS and if the the callback function correctly sends the operational status signal to can.
 */
...
/**
 * @test This test case checks if after receiving RR_RPM from the CAN bus, the values are correctly published to the correct ROS TOPIC, the test node is used to subscribe to the respective topic and read the values published and verify them
 */
TEST_F(RosCanTest, TestCanInterpreter_TEENSY_C1_RR_RPM_CODE) {
    ...
    auto rr_rpm_pub_sub = test_node_->create_subscription<custom_interfaces::msg::WheelRPM>(
            topics_["right_rear"], 10, [&](const custom_interfaces::msg::WheelRPM::SharedPtr msg) {
                ...
                EXPECT_TRUE(isApproxEqual(msg->rr_rpm, 164.15, tolerance));
            });
...
TEST_F(RosCanTest, TestOutOfRangeUpperSteeringThrottle) {
    prepareOutOfRangeValues(STEERING_UPPER_LIMIT + 1, STEERING_UPPER_LIMIT + 1, 0);
    ros_can_->controlCallback(control_command_);
}

TEST_F(RosCanTest, TestOutOfRangeLowerSteeringThrottle) {
    prepareOutOfRangeValues(STEERING_LOWER_LIMIT - 1, STEERING_LOWER_LIMIT - 1, 0);
    ros_can_->controlCallback(control_command_);
}
...
TEST_F(RosCanTest, TestAliveMsgCallback) {
    ...
    EXPECT_CALL(*mock_can_lib_wrapper_, canWrite(testing::_, testing::_, testing::_, testing::_, testing::_))
            .Times(3)
...
/**
 * @test This test case checks if the the method correctly changes the wheels steering angle to the actuator steering angle.
 */
